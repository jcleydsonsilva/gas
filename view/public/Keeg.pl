...
# Iniciang Tradution the page
	...
	print $page->start_html("Metabolic Pathways Analysis");
...
# Header of cadaster
	print '<center><H2><br>';
		print 'Metabolic Pathways Analysis';
	print '</center></H2>';
	...
	if($page->param('idIsolate') ne '' && $page->param('action') eq '')
	{
		...
		print '<fieldset>';
		print '<legend> Metabolic pathways </legend>';
		print '<table>';
	...
	if($page->param('query') eq 'Search')
	{
		...
		$resultado = KeegController::RunKeeg($page->param('isolate'));
