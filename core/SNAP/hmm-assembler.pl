...
use vars qw(
	...
	$opt_a $opt_3 $opt_p $opt_5 $opt_t $opt_Z
);
getopts('roxi:e:A:D:M:S:C:I:N:a:3:p5:t');
...
options:
  ...
  -Z  <species>          sets species-specific values
" unless @ARGV == 2;

# boost syntax is '+' after the model eg. 0:12+ or 3+
...
if ($SPECIES) {
	die "species is not yet supported\n";
}
